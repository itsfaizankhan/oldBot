...
class Test3(commands.Cog):
	...
	async def on_ready(self):
		...
		await sheep_channel.send(f'Extension loaded: Test3')
	...
	async def among_us(self, ctx, imap: str, p_limit: int, im_limit: int, room_code: str, *, vc: str):
		...
		embed = discord.Embed(title="Matchmaking", desciption = f"Among Us announcement Command", colour=member.colour)
	...
	async def among_us_handler(self, ctx, error):
		if isinstance(error, commands.MissingRequiredArgument):
			...
			embed=discord.Embed(title="You missed something!", color=ctx.author.colour)
			embed.add_field(name="Last missing value", value=f"{miss_para}", inline=True)
			await ctx.send(embed=embed)
