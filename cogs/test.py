...
class Test(commands.Cog):
    ...
    async def test(self, ctx, *, member: discord.Member=None):
        if not member:
            member = ctx.author
        embed = discord.Embed(
            title="Title", desciption="Discription\n\nBlek", colour=member.colour)
        embed.add_field(name="Blek field name:",
                        value="Blek value", inline=True)
        embed.add_field(name="Blek field2 name:",
                        value="Blek value2", inline=True)
