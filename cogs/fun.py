...
fact_animal = ["dog", "cat", "panda", "racoon",
               "kangaroo", "fox", "bird", "koala"]

image_animal = ["dog", "cat", "panda", "koala", "fox",
                "kangaroo", "racoon", "whale", "bird", "pikachu"]

overlays = ["gay", "glass", "wasted"]

filters = ["greyscale", "invert", "brightness", "threshold",
           "sepia", "red", "green", "blue", "blurple", "pixelate", "blur"]
...
class Fun(commands.Cog):
        ...
        if (animal := animal.lower()) in fact_animal:
            ...
            async with request("GET", fact_url, headers={}) as response:
                if response.status == 200:
                    ...
                    embed.set_author(
                        icon_url=ctx.author.avatar_url, name=str(ctx.author))
            ...
            embed = discord.Embed(
                title=f"No facts are available for \'{animal}\'.", color=discord.Colour.red())
            await ctx.send(embed=embed)
    ...
    async def animal_image(self, ctx, *, animal: str):
        if (animal := animal.lower()) in image_animal:
            ...
            embed = discord.Embed(
                title=f"No images are available for \'{animal}\'.", color=discord.Colour.red())
            await ctx.send(embed=embed)
        ...
        async with request("GET", pat_url, headers={}) as response:
            if response.status == 200:
                ...
                if member == ctx.author:
                    embed = discord.Embed(
                        title=f"There there {member}", colour=rand_colour)
                else:
                    embed = discord.Embed(
                        title=f"{ctx.author} pats {member}", colour=rand_colour)
                ...
                rand_colour = random.randint(0, 16777215)
                embed = discord.Embed(title=f"No pat media available.",
                                      description="Let me pat you! there there~ *pats*", colour=rand_colour)
                await ctx.send(embed=embed)
    ...
    async def hug_gif(self, ctx, *, member: discord.Member = None):
        ...
        async with request("GET", hug_url, headers={}) as response:
            if response.status == 200:
                ...
                if member == ctx.author:
                    embed = discord.Embed(
                        title=f"Hugs for {member}!", colour=rand_colour)
                else:
                    embed = discord.Embed(
                        title=f"{ctx.author} hugs {member}", colour=rand_colour)
                ...
                rand_colour = random.randint(0, 16777215)
                embed = discord.Embed(title=f"No hug media available.",
                                      description="I'll hug you! *hugs*", colour=rand_colour)
                await ctx.send(embed=embed)
    ...
    async def meme_gif(self, ctx):
        ...
        async with request("GET", meme_url, headers={}) as response:
            if response.status == 200:
                ...
                embed = discord.Embed(
                    title=f"Requested by: {ctx.author}", description=f"{caption}", colour=rand_colour)
                ...
                if meme_link is not None:
                    ...
                    embed = discord.Embed(
                        title="Something went wrong while loading the image.", colour=rand_colour)
                    await ctx.send(embed=embed)

            else:
                embed = discord.Embed(
                    title=f"Something went wrong. Better luck next time ಡ ͜ ʖ ಡ", colour=discord.Colour.red())
                await ctx.send(embed=embed)
    ...
    async def av_overlay(self, ctx, overlay: str, *, member: discord.Member = None):
        ...
        if (overlay := overlay.lower()) in ("gay", "wasted", "glass"):
            ...
            embed = discord.Embed(
                title=f"No overlay named '{overlay}' available.", color=discord.Colour.red())
            await ctx.send(embed=embed)
    ...
    async def canvas_list(self, ctx, *, arg=None):
        ...
        if arg == None:
            embed.add_field(name="Available Animal Names for Facts",
                            value=fact_animal, inline=False)
            embed.add_field(name="Available Animal Names for Images",
                            value=image_animal, inline=False)
            embed.add_field(name="Available Overlays",
                            value=overlays, inline=False)
            embed.add_field(name="Available Filters",
                            value=filters, inline=False)
            await ctx.send(embed=embed)
            ...
            embed = discord.Embed(
                title="Seems like you've typed something wrong.", colour=discord.Colour.red())
            await ctx.send(embed=embed)
